...
import com.bedrosians.bedlogic.domain.product.enums.Body;
import com.bedrosians.bedlogic.domain.product.enums.DesignLook;
import com.bedrosians.bedlogic.domain.product.enums.DesignStyle;
import com.bedrosians.bedlogic.domain.product.enums.Edge;
import com.bedrosians.bedlogic.domain.product.enums.Grade;
import com.bedrosians.bedlogic.domain.product.enums.MpsCode;
import com.bedrosians.bedlogic.domain.product.enums.Status;
...
public class ProductDaoImpl extends GenericDaoImpl<Product, String> implements ProductDao {
	...
	public Product getItemById(Session session, final String itemId) {
    	Query query = session.createQuery("From Product where itemcode = :itemCode");
	...
	*/
	...
	public List<Product> getItemsByQueryParameters(MultivaluedMap<String, String> queryParams){
	   ...
	   DetachedCriteria itemCriteria = DetachedCriteria.forClass(Product.class);
	   ...
	   while(it.hasNext()) {
    		...
    		//------- add association criteria --------//
	 	   	if(ProductNewFeature.allProperties().contains(key)) {	
	   		   if(newFeatureCriteria == null)
	     	      newFeatureCriteria = itemCriteria.createCriteria("productNewFeature", JoinType.LEFT_OUTER_JOIN);
	 	   	...
	 	   	switch(key) {
			       ...
			       vendorCriteria.add(Restrictions.eq("productVendorId.vendorId", Integer.parseInt(value)));
			       break;
		...
		try {
			if(maxResults > 0)
		       products =  (List<Product>)itemCriteria.getExecutableCriteria(getSession()).setLockMode(LockMode.NONE).setFlushMode(FlushMode.COMMIT).setMaxResults(maxResults).setCacheable(true).list();//executeCriteria(itemCriteria);//(List<Product>)itemCriteria.list();			
			else
			   products =  (List<Product>)itemCriteria.getExecutableCriteria(getSession()).setLockMode(LockMode.NONE).setFlushMode(FlushMode.COMMIT).setCacheable(true).list();	
		}
		catch(Exception e){
		     throw e;	
		}
	...
	public void deleteItem(Product product){
		delete(sessionFactory.getCurrentSession(), product); 
	}
